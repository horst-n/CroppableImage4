...
class CroppableImage4CropSetting {  //extends WireData {
    ...
    /**
     * create a CroppableImage4CropSetting instance from a crop setting line
     * @param  string $cropSettingLine       example: crop-name,200,200,basic-page
     * @return CroppableImage4CropSetting
     */
    public static function createFromString($cropSettingLine) {
        ...
        $cropSetting = new CroppableImage4CropSetting(
...
class CroppableImage4CropSettings {
    ...
    /**
     * add an item either via string or CroppableImage4CropSetting instance
     * @param string|CroppableImage4CropSetting $setting
     */
    public function addItem($setting) {
        if ($setting instanceof CroppableImage4CropSetting) {
            $this->items[$setting->name] = $setting;
            $this->names[$setting->name] = $setting->name;
        } else if (trim((string) $setting)) {
            if (CroppableImage4CropSetting::isValidString($setting)) {
                $cropSetting = CroppableImage4CropSetting::createFromString($setting);
     ...
     * @return Boolean|CroppableImage4CropSettings   either false if no setting is found for template
     *                                              or a CroppableImage4CropSettings instance with the matching items
     */
    public function getCropSettingsForTemplate($templateName=null) {
        ...
        $cropSettings = new CroppableImage4CropSettings();
     ...
     * @param  string|object(Template) $templateName
     * @return Boolean|CroppableImage4CropSetting    either false if no setting is found by that name and for template
