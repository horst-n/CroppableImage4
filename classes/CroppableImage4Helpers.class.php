...
class CroppableImage4Helpers extends WireData {


    const ciVersion = 200;

    const ciGlobalConfigName = 'FieldtypeCroppableImage4';
    ...
    static public function getTemplate($templateName, $array = null) {
        //                                                                   CroppableImage4 !
        $template = new TemplateFile(dirname(__FILE__) . "/../templatefiles/{$templateName}4.php");  // change 3 to 2 for NoneNameSpacedVersion !!  OBSOLETE WITH CAI4  !!
        if($array) {
    ...
    */
    static public function writeModuleConfigData($classname, &$data) {
        $logs = $this->config->logs;                              // get current log status
        if(!is_array($logs) || !isset($logs['modules'])) {
            $this->modules->saveModuleConfigData($classname, $data);
            return;
        }
        $this->config->logs = array();                            // switch off logging for modules
        $this->modules->saveModuleConfigData($classname, $data);  // save config data
        $this->config->logs = $logs;                              // toggle on logging for modules
    }
    ...
    /**
    * Merge Options before they get passed to Pageimage::size
    *
    * @param pageimage $img
    * @param string $suffix
    * @param int $width
    ...
    * @return pageimage
    */
    static public function mergeFinalOptions($suffix, $options, $x, $y, $w, $h) {
        // build cropExtra array if needed
        if($w > 0 && $h > 0) $options['cropExtra'] = array($x, $y, $w, $h);
    ...
    /**
     * easy access to the ImageOptions of CroppableImage4,
     *
     * @return array
     */
    static public function getGlobalImageSettings($asArray = false) {
        $o = wire('modules')->get(self::ciGlobalConfigName);
        $options = $o->getCroppableImage4Options();
        if(!is_array($options)) {
            throw new WireException(__('Unable to get CroppableImage4Options!'));
        }
        return true === $asArray ? $options : self::arrayToObject($options);
    }
     ...
     * @return array
     */
    static public function getGlobalImageSetting($key) {
    ...
    /**
     * easy access to the global settings of CroppableImage4,
     * stored in the Modules-Configpage of InputfieldCroppableImage4
     *
    ...
    static public function getGlobalConfigSettings($asArray = false) {
        ...
        if(0 == count($gcd)) {
            $fetchedQuality = isset(wire()->config->imageSizerOptions['quality']) && is_int(wire()->config->imageSizerOptions['quality']) ? wire()->config->imageSizerOptions['quality'] : 90;
            $fetchedWebpQuality = isset(wire()->config->imageSizerOptions['webpQuality']) && is_int(wire()->config->imageSizerOptions['webpQuality']) ? wire()->config->imageSizerOptions['webpQuality'] : 90;
            ...
            $gcd = array(
                ...
                'optionQuality' => $fetchedQuality,
                'optionSharpening' => 1, // 'soft'
                'caiWebpAdd' => false,
                'caiWebpQuality' => $fetchedWebpQuality
            );
        }
