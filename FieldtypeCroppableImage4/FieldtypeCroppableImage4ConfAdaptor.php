...
class FieldtypeCroppableImage4ConfAdaptor extends Wire {
    ...
    public function getConfig(array $data) {
        ...
        $fieldset = $modules->get('InputfieldFieldset');
        $fieldset->label = 'WebP Support';
        $fieldset->attr('name+id', '_caiwebpsupport');
        $fieldset->description = $this->getText('fieldsetDescriptionWebp');
        $fieldset->collapsed = Inputfield::collapsedNo;

            $field = $modules->get('InputfieldCheckbox');
            $field->attr('name+id', 'caiWebpAdd');
            $field->label = $this->getText('caiWebpAdd_Label');
            $field->notes = $this->getText('caiWebpAdd_Notes');
            $field->attr('value', 1);
            $field->attr('checked', (isset($data['caiWebpAdd']) && $data['caiWebpAdd'] ? 'checked' : ''));
            $field->columnWidth = 50;
            $fieldset->add($field);

            $field = $modules->get('InputfieldInteger');
            $field->attr('name+id', 'caiWebpQuality');
            $field->label = $this->getText('caiWebpQuality_Label');
            $field->attr('value', (isset($data['caiWebpQuality']) && $data['caiWebpQuality']>0 && $data['caiWebpQuality']<=100 ? $data['caiWebpQuality'] : 90));
            $field->description = $this->getText('caiWebpQuality_Description');
            $field->columnWidth = 50;
            $field->showIf = "caiWebpAdd=1";
            $fieldset->add($field);
